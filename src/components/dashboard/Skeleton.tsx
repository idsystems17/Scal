export function SkeletonBlock({ width = '100%', height = 16, radius = 6 }: { width?: number | string; height?: number; radius?: number }) {
  return <div className="skeleton-pulse" style={{ width, height, borderRadius: radius, background: '#c4c9d0' }} />
}

export function SkeletonCard({ children, padding = 24 }: { children: React.ReactNode; padding?: number }) {
  return (
    <div style={{ background: 'white', border: '1px solid #c4c9d0', borderRadius: 16, padding, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
      {children}
    </div>
  )
}
