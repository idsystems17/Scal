...
export default async function AdminIntegracaoPage() {
  ...
  const empresas = (clientes ?? []).map(c => ({
    id: c.id as string,
    nome: (c.nome_loja as string) ?? 'Empresa',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        <h2 style={{ fontSize: 20, fontWeight: 700, color: '#0f172a', margin: 0 }}>Integração — checkout próprio</h2>
        <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>
          Para empresas que não usam Kiwify/Hotmart/Eduzz — gere e envie as instruções técnicas prontas para o dev delas
        </p>
      </div>
